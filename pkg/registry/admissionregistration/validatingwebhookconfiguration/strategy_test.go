...
func TestStaticSuffixWarningsAndValidation(t *testing.T) {
	...
	t.Run("feature gate enabled rejects static suffix on update", func(t *testing.T) {
		featuregatetesting.SetFeatureGateDuringTest(t, utilfeature.DefaultFeatureGate, features.ManifestBasedAdmissionControlConfig, true)
		errs := Strategy.ValidateUpdate(ctx, makeConfig(staticName), makeConfig(staticName))
		found := false
		for _, e := range errs {
			if e.Field == "metadata.name" {
				found = true
		...
		if !found {
			t.Error("Expected validation error for .static.k8s.io suffix on update when feature gate is enabled")
		}
	})
}
