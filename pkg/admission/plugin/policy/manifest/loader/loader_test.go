...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Load manifests
			result, err := LoadManifestsFromDirectory(dir)
...
func TestGetPoliciesWithBindings(t *testing.T) {
	// Create temp directory
	dir := t.TempDir()
...
kind: ValidatingAdmissionPolicyBinding
metadata:
  name: test-binding-1.static.k8s.io
spec:
  policyName: test-policy.static.k8s.io
  validationActions:
  - Deny
...
kind: ValidatingAdmissionPolicyBinding
metadata:
  name: test-binding-2.static.k8s.io
spec:
  policyName: test-policy.static.k8s.io
  validationActions:
  - Warn
`
	...
	if err := os.WriteFile(filepath.Join(dir, "policy.yaml"), []byte(policyYAML), 0644); err != nil {
		t.Fatalf("failed to write policy: %v", err)
	}
	if err := os.WriteFile(filepath.Join(dir, "binding1.yaml"), []byte(binding1YAML), 0644); err != nil {
		t.Fatalf("failed to write binding1: %v", err)
	}
	if err := os.WriteFile(filepath.Join(dir, "binding2.yaml"), []byte(binding2YAML), 0644); err != nil {
		t.Fatalf("failed to write binding2: %v", err)
	}
	...
	pwbs := result.GetPoliciesWithBindings()
	if len(pwbs) != 1 {
		t.Fatalf("expected 1 policy with bindings, got %d", len(pwbs))
	}

	if pwbs[0].Policy.Name != "test-policy.static.k8s.io" {
		t.Errorf("expected policy name 'test-policy.static.k8s.io', got %q", pwbs[0].Policy.Name)
	}

	if len(pwbs[0].Bindings) != 2 {
		t.Errorf("expected 2 bindings, got %d", len(pwbs[0].Bindings))
	}
}
