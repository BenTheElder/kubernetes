...
`,
			...
			wantErr:     true,
			errContains: "MutatingWebhookConfiguration but this directory is configured for ValidatingAdmissionWebhook",
		},
		{
...
`,
		...
		{
			name: "webhook with missing caBundle",
			files: map[string]string{
				"webhook.yaml": `
apiVersion: admissionregistration.k8s.io/v1
kind: ValidatingWebhookConfiguration
metadata:
  name: no-ca.static.k8s.io
webhooks:
- name: test.webhook.io
  admissionReviewVersions: ["v1"]
  clientConfig:
...
`,
			...
			wantErr:     true,
			errContains: "clientConfig.caBundle is required",
		},
	}
...
`,
				...
				wantErr:     true,
				errContains: "MutatingWebhookConfiguration in List; only ValidatingWebhookConfiguration is supported",
			},
			{
