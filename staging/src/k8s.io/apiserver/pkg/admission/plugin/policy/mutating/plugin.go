...
func (a *Plugin) SetManifestLoaders(loaders *initializer.ManifestLoaders) {
	...
	a.SetStaticSourceFactory(func(manifestsDir string) (generic.Source[PolicyHook], func(ctx context.Context) error, error) {
		staticSource := source.NewStaticMutatingPolicySource[PolicyEvaluator](manifestsDir, a.GetAPIServerID(), compilePolicy,
			func(dir string) ([]*v1.MutatingAdmissionPolicy, []*v1.MutatingAdmissionPolicyBinding, []byte, error) {
				return loadFunc(dir)
			})
