...
func RecordAutomaticReloadSuccess(manifestType ManifestType, apiServerID, configData string) {
	apiServerIDHash := getHash(apiServerID)
	...
	RecordLastConfigInfo(manifestType, apiServerID, configData)
}

// RecordLastConfigInfo records the hash of the last successfully loaded configuration.
func RecordLastConfigInfo(manifestType ManifestType, apiServerID, configData string) {
	if provider, ok := configHashProviders[manifestType]; ok {
		...
		provider.SetHashes(getHash(apiServerID), getHash(configData))
	}
}
